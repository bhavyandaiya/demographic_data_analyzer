...
def calculate_demographic_data(print_data=True):
    ...
    country_income = country_salary.groupby('native-country')['salary'].apply(lambda x: (x == '>50K').mean() * 100)

    highest_earning_country = country_income.idxmax()
    highest_earning_country_percentage = country_income.max()

    # Identify the most popular occupation for those who earn >50K in India.
    IN_salary_occupation = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]
    top_IN_occupation = IN_salary_occupation['occupation'].value_counts().idxmax()
